...
def get_new_user(email: str, user_key_size: int = USER_KEY_SIZE, directory_url: str = DIRECTORY_URL):
    ...
    while True:
        tos_reply = input(
            f'Do you agree to the ToS of Let\'s Encrypt located at {user.directory.meta.terms_of_service} ? (Y/N): ').lower()
        if tos_reply == 'y':
            break
        elif tos_reply == 'n':
            exit('You must agree to the ToS of Let\'s Encrypt in order to generate an SSL certificate for webhooks.')
    logger.info("Registering")
    regr = user.new_account(messages.NewRegistration.from_data(
...
def http01_validate(user: client.ClientV2, challenge: challenges.HTTP01, order: messages.OrderResource, port: int = PORT):
    ...
    print(resource.chall)
    print(resource.chall.token)
    f = open("token", "wb+")
    f.write(resource.chall.token)
    f.close()
...
class cert_manager():
    ...
    def setup_cert(self):
        ...
        if os.path.isfile(self.fullchain_path):
            ...
            if(cert_expiration_datetime(fullchain).timestamp() - 86400 < time.time()):  # change to 1 month
                need_cert = True
            else:
                need_cert = False
        else:
    ...
    def renew(self):
        user, regr = user_load()
        privkey, fullchain = get_certs(
            user, self.domain, key_pem=self.read_privkey())
        self.write_fullchain(fullchain)

    # def renew_loop(self, callback=None):
    #     while True:
    #         expiration = cert_expiration_datetime(
    #             self.read_fullchain()).timestamp() - 86400
...
a = cert_manager("maximusjshokes+testing2@gmail.com",
                 "www.butopia.com.ng")
# a = cert_manager("maximusjshokes+testing2@gmail.com",
#                  "1caf-102-89-34-207.ngrok.io")
